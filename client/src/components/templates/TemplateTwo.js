...
function TemplateTwo({ resume }) {
  ...
  return (
    <>
      ...
      <div
        ref={resumeRef}
        className="w-[794px] h-[1123px] mx-auto bg-white shadow-xl font-sans text-gray-900 overflow-hidden flex"
      >
        {/* Left Sidebar */}
        <aside className="w-[30%] bg-gray-100 p-6 border-r space-y-6">
          ...
          <section>
            ...
            <div className="mb-2">
              ...
              <p className="text-sm text-gray-800 break-words">{resume.email}</p>
            </div>
            <div>
              <h3 className="text-xs font-bold text-gray-700 uppercase">Phone</h3>
              <p className="text-sm text-gray-800 break-words">{resume.phone}</p>
            </div>
          </section>
          ...
          {resume.skills && typeof resume.skills === 'object' && (
            <section className="mb-6">
              <h2 className="text-lg font-bold text-black border-b border-gray-300 pb-1 mb-3 uppercase">Skills</h2>
                  ...
                  <div key={category} className="mb-2">
                    ...
                    <ul className="flex flex-wrap gap-2 text-sm">
                      {skillList.map((skill, index) => (
                        <li key={index} className="bg-blue-200 text-black rounded-full px-3 py-1">{skill}</li>
                      ))}
                    </ul>
                  </div>
                ) : null
